...
| Deprecated `Clock` usage and a second elapsed-time read could skew frame-derived timing. | **2026-07-17 follow-up:** `SimulationScheduler` now owns browser timestamps, bounded frame deltas, fixed physics, city/UI/gameplay/render/paused clocks, and the minimap render clock; `main.js` owns no delta or elapsed-time accumulator. | `test/SimulationScheduler.test.js`, production build, and Playwright WebGL smoke/transition soak. |
