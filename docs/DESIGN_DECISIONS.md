...
| DD-016 | Port MetroPulse to Godot 4.6 .NET as a desktop-first C# application for Windows, macOS, and Linux. Keep the browser build as the frozen reference/legacy build, keep compatibility content in engine-neutral JSON, and migrate browser saves only through an explicit export/import bridge. Mobile and C# web export are not release targets. | Provisional — product signoff required | Product Lead | 2026-08-14 | Phase 1 may establish the desktop Godot shell, but final cutover and any claim that Phase 0 has exited remain blocked until the Product Lead accepts or supersedes this entry. |
